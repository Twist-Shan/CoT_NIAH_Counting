...
out=[]
for cid,mode,assay in [('category_count_seed1254_city3_targetcity','nonthinking','broad'),('category_count_seed1254_city3_targetcity','native_thinking','targeted')]:
 p=next(p for p in read(root/'plans/category_Gemma4-E4B.json') if p['case_id']==cid and p['mode']==mode)
 ...
 saved=read(root/'full/Gemma4-E4B/category/causal'/f'{mode}_{cid}.json')['assays'][assay]['arms'][0]['generation']
...
(base/('aligned_clean_diagnostic_fixed.json' if '--fixed' in sys.argv else 'aligned_clean_diagnostic.json')).write_text(json.dumps(out))
