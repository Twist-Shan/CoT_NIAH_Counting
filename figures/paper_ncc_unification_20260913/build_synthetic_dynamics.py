...
def save_combined(fig, _stem, _title):
    ...
    for item in fig.texts:
        ...
        # Leave a visible gap below the heatmap tick labels.
        extra_gap = .012 if item.get_text() == 'Training step' else 0
        item.set_position((x, offset + ratio*y - extra_gap))
    ...
    data = original.read(OUT / 'data/synthetic_ncc_dynamics.csv')
    ...
    for i, title in enumerate(['E. Running index', 'F. Final count']):
        ...
        for mode, color, label in [('nonthinking', original.NT, 'Non-thinking'),
            ...
            assert layer == selected[mode][subset.endpoint.iloc[0]]
    ...
    (OUT / 'joint_dynamics_manifest.json').write_text(json.dumps({
        'change': 'Discovery NCC-selected layers throughout training; attention heatmaps unchanged.',
        'data_unchanged': False, 'attention_data_unchanged': True, 'selection_file': 'selection.json', 'source_sha256': original.SOURCES,
