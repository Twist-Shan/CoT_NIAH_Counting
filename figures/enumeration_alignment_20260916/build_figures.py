...
def curve(ax, x, stats, color, *, linestyle="-", marker=None, alpha=.13):
    ...
    assert all(v is not None and np.isfinite(v) for v in y)
    ci = np.asarray([s["ci95"] for s in stats], dtype=float)
...
def update(report):
    ...
    for col, mode in enumerate(MODES):
        for mi, (model, _, color) in enumerate(MODELS):
            ...
            for di, direction in enumerate(("forward", "backward")):
                ...
                for j, scope in enumerate(("endpoint", "four_token_tail", "item_span")):
                    ...
                    records.append(dict(model=model, mode=mode, direction=direction, scope=scope,
                ...
                # Keep a common four-step horizon so a shrinking denominator cannot make the curve rise.
                source = REPO / "work/en10u/src/fresh_n10_update_v1"
                assembly = read(source / "assembly_audit.json")
                trials = []
                for k in (4, 6):
                    path = source / "primary" / model / mode / f"k{k}_{direction}_item_span/trials.jsonl"
                    digest = hashlib.sha256(path.read_bytes()).hexdigest()
                    suffix = f"/fresh_n10_update_v1/primary/{model}/{mode}/k{k}_{direction}_item_span/trials.jsonl"
                    expected = [v for p, v in assembly["source_files_sha256"].items() if p.endswith(suffix)]
                    assert expected == [digest]
                    SOURCES[path.relative_to(ROOT).as_posix()] = digest
                    with path.open(encoding="utf-8") as handle:
                        trials.extend(r for line in handle if line.strip() if (r := json.loads(line))["condition"] == "donor_to_receiver")
                assert len(trials) == 20
                hops = [continuation_summary(trials, h, draws=10000, random_seed=20260915) for h in range(1, 5)]
                curve(axes[1][col], list(range(1, 5)), [h["unconditional"] for h in hops], color, linestyle=ls, marker=marker)
                records.extend(dict(model=model, mode=mode, direction=direction, scope="item_span", metric="unconditional_prefix",
                                    hop=h["hop"], numerator=h["successes"], denominator=h["horizon_eligible"],
                                    conditional_denominator=h["conditional_eligible"], **h["unconditional"]) for h in hops)
        ax = axes[0][col]
        ...
        ax.yaxis.set_major_formatter(PercentFormatter(1, decimals=0))
        style.panel(ax, f'{"CD"[col]}. {mode.split("_")[-1].title()}: continued prefix', "Prefix success" if col == 0 else None, "Continuation depth")
    models_legend(fig)
...
def readout(answer, blanking, relay):
    fig, axes = style.four_panels(height=4.0)
    records = []
    for col, mode in enumerate(MODES):
        ax = axes[0][col]
    ...
    for fi, mode in enumerate(MODES):
        for mi, (model, _, color) in enumerate(MODELS):
            x = fi * 2 + mi
            for ci, (condition, marker) in enumerate((("clean", "o"), ("prompt_records_blank", "s"), ("trace_all_blank", "^"))):
                stat = cell(blanking, model, mode)["conditions"][condition]["accuracy"]
                point(axes[1][0], x + (ci-1)*.18, stat, color, marker)
            ...
            estimate = next(r for r in cell(relay, model, mode)["estimates"] if
                            r["width"] == 4 and r["scope"] == "all_eligible" and r["reset"] == "post_terminal_suffix_clean_reset")
            stat = {"estimate": estimate["signed_reduction"], "ci95": estimate["ci95"]["signed_reduction"]}
            point(axes[1][1], x, stat, color)
            records.append(dict(model=model, mode=mode, metric="signed_suffix_damage_reduction", **estimate))
    for ax in axes[1]:
        ax.set(xticks=[.5, 2.5], xticklabels=["Index", "Bullet"], xlim=(-.5, 3.5))
        ax.yaxis.set_major_formatter(PercentFormatter(1, decimals=0))
    axes[1][0].set(ylim=(-.05, 1.08), yticks=[0, .5, 1])
    axes[1][1].set(ylim=(.35, 1.6), yticks=[.5, 1, 1.5])
    axes[1][1].axhline(1, color=".5", ls=":", lw=.8)
    style.panel(axes[1][0], "C. Trace blanking", "Exact-count accuracy")
    style.panel(axes[1][1], "D. Clean suffix reset", "Signed damage reduction")
    models_legend(fig)
    fig.legend([Line2D([], [], color=".25", marker=m, ls="", ms=4) for m in ("o", "s", "^")],
               ["Clean", "Record blank", "Trace blank"], loc="center", bbox_to_anchor=(.5, .03), ncol=3)
...
if __name__ == "__main__":
    ...
    reports = {name: read(base / directory / "audit.json") for name, directory in (
        ("retrieve", "retrieve_audit_1550"), ("answer", "answer_patch_audit_1545"), ("read", "read_audit_v1"), ("relay", "relay_audit_1450"))}
    reports["update"] = read(args.update_audit.resolve())
    assert all(r["status"] == "PASS" for r in reports.values())
    assert reports["update"]["schema"] == "enumeration_n10_update_audit_v1"
    retrieve(reports["retrieve"])
    head_scores(reports["retrieve"])
    update(reports["update"])
    readout(reports["answer"], reports["read"], reports["relay"])
    ...
    (OUT / "manifest.json").write_text(json.dumps({"status": "PASS", "figures": FIGURES, "source_sha256": SOURCES,
