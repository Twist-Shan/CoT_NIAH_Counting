...
PAPER=WORK/'runs/paper_figures/figures/synthetic_appendix/paper_checked_20260908'
...
def continuation():
    ...
    a.set_title('A. Activation patching at L1',loc='left',fontsize=11,pad=8)
    ...
    a.text(.52,.57,'patch activations',fontsize=9.5,ha='left',va='center',color=INK)
    a.text(.48,.04,'Two positions: <Sep> + marker',fontsize=9.5,ha='center')
    a.text(.48,-.08,'Then generate the continuation',fontsize=9,color=INK,ha='center')
    ...
    panel(ax,'B. Continuation exact match','Exact match (%)')
...
def answer():
    ...
    fig.legend(handles,labels,loc='lower left',bbox_to_anchor=(.05,.005),ncol=3,
               fontsize=9,columnspacing=.7,handlelength=1.,handletextpad=.35)
    ax=axs[1]
    for mode,color,label in [('nonthinking',NT,'Non-thinking'),('thinking',T,'Thinking')]:
        f=b[b['mode'].eq(mode)].sort_values('layer')
        assert f.pairs.eq(180).all()
        ax.plot(f.layer,100*f.adoption,'o-',color=color,label=label)
    panel(ax,'B. Answer activation patching','Donor answer match (%)')
    ax.set(xticks=[1,2,3,4],xlabel='Layer of activation patching',ylim=(0,105),yticks=[0,25,50,75,100])
