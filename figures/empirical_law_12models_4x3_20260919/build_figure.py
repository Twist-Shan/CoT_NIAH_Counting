"""Render the appendix's 12-group grid in four columns and three rows.
...
def main():
    ...
    plt.rcParams.update({
        ...
        'font.size': 8.5, 'axes.labelsize': 9, 'axes.titlesize': 9.2,
    ...
    for i, (slot, title) in enumerate(ORDER):
        ...
        for mode, _, ls, marker in MODES:
            for length, color in zip(lengths, colors):
                ...
                ax.scatter(g.N, g.observed, s=5.3 if mode == 'direct' else 6.5,
                           ...
                           alpha=.78, zorder=3)
        ax.set_title(f'{chr(65+i)}  {title}', loc='left', fontsize=9.2, fontweight='normal', pad=5)
        ...
        ax.tick_params(length=3, width=.65, pad=2, labelleft=(col == 0), labelbottom=(row == 2))
        ax.minorticks_off()
    ...
    fig.legend(handles=mode_handles, loc='lower center', bbox_to_anchor=(.50, .006),
    ...
    manifest = dict(rows=3, columns=4, comparison_groups=12, modes=2, lengths=lengths,
                    ...
                    fonts_pt=dict(title=9.2, axis_label=9, ticks=8, legend=8.5),
