...
Run with python. Then export nonthinking_section_v6.drawio.
...
ax.legend([Line2D([],[],marker='o',color=INK,ls='',ms=4),Line2D([],[],marker='D',color=INK,ls='',mfc='white',ms=4)],['Top-$K$','Random'],loc='lower center',bbox_to_anchor=(.5,1.025),ncol=2,fontsize=11.5,handlelength=.7,handletextpad=.35,columnspacing=.8,borderpad=0,borderaxespad=0)
ax=axes[2]
for m,c in zip(MODELS,COLORS):
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v6.{ext}',dpi=220)
...
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v6_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v6.drawio',encoding='utf-8',xml_declaration=True)
...
svg=(OUT/'result_panels_v6_outlined.svg').read_bytes()
...
tree.write(OUT/'nonthinking_section_v6.drawio',encoding='utf-8',xml_declaration=True)
