...
Run with python. Then export nonthinking_section_v5.drawio.
...
panel_starts=[.012,.390,.636]
axis_rects=[[.080,.210,.285,.57],[.450,.210,.165,.57],[.700,.210,.285,.57]]
...
ax.set(xlim=(-.45,1.45),ylim=(-.08,2.5),xticks=[0,1],xticklabels=['Qwen','Gemma'],yticks=[0,1,2],ylabel='Absolute count shift')
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v5.{ext}',dpi=220)
...
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v5_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v5.drawio',encoding='utf-8',xml_declaration=True)
# Equal column pitch; compact computational rows and more room for token-to-state arrows.
cells={c.get('id'):c for c in root.findall('mxCell')}
columns=[330,566,802,1038,1274]
rows_y={'high':64,'mid':114,'low':164,'tokens':246,'positions':296}
...
for index,n in enumerate([1,2,3,10]):
    ...
    place('low-'+str(n),cx,rows_y['low'],132,36,26)
...
for row in ['low','mid','high']:place('answer-'+row,columns[-1],rows_y[row],132,36,26)
...
tree.write(OUT/'mechanism_matched_v5.drawio',encoding='utf-8',xml_declaration=True)
model.set('pageWidth','1700');model.set('pageHeight','781')
...
svg=(OUT/'result_panels_v5_outlined.svg').read_bytes()
uri='data:image/svg+xml,'+base64.b64encode(svg).decode()
vertex('section-results','',f'shape=image;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;image={uri};',0,330,1700,450.5)
tree.write(OUT/'nonthinking_section_v5.drawio',encoding='utf-8',xml_declaration=True)
...
manifest['layout']={'version':5,'result_axes':axis_rects,'axis_font_pt':12,'tick_font_pt':11.5,'legend_font_pt':11.5,'title_font_pt':13,'font_reference_width_inches':10,'at_7_inch_width':{'axis_pt':8.4,'ticks_and_legend_pt':8.05,'title_pt':9.1},'mechanism_column_centers':columns,'mechanism_row_centers':rows_y,'mechanism_text_size':24,'mechanism_math_size':26,'composition':{'width':1700,'mechanism_height':330,'results_height':450.5},'change_from_v4':{'upper_row_pitch':[56,50],'low_to_token_pitch':[56,82],'token_to_position_pitch':[40,50],'BD_axes_width_increase':.285/.240-1,'C_axes_width_reduction':1-.165/.240,'result_axes_height_reduction':1-(2.65*.57)/(3.1*.61)}}
