...
Run with python. Then export nonthinking_section_v7.drawio.
...
ax.set(xlim=(-.5,41.5),ylim=(-.45,3.35),xticks=[0,10,20,30,40],yticks=[0,1,2,3],xlabel='Intervention layer',ylabel='Corruption effect')
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v7.{ext}',dpi=220)
...
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v7_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v7.drawio',encoding='utf-8',xml_declaration=True)
...
svg=(OUT/'result_panels_v7_outlined.svg').read_bytes()
...
tree.write(OUT/'nonthinking_section_v7.drawio',encoding='utf-8',xml_declaration=True)
