...
Run with python. Then export nonthinking_section_v4.drawio.
...
fig=plt.figure(figsize=(10,3.10),facecolor='white')
panel_starts=[.012,.341,.670]
axis_rects=[[x+.070,.180,.240,.61] for x in panel_starts]
...
for i,(m,c) in enumerate(zip(MODELS,COLORS)):
    for cond,dx,marker in [('ranked',.11,'o'),('layer_matched_random',-.11,'D')]:
        ...
        ax.errorbar(i+dx,y,yerr=[[y-lo],[hi-y]],fmt=marker,color=c,ms=5.2,capsize=3,lw=1.2,mfc=c if cond=='ranked' else 'white',mew=1.2)
ax.set(xlim=(-.45,1.45),ylim=(-.08,2.5),xticks=[0,1],xticklabels=['Qwen ($K=32$)','Gemma ($K=6$)'],yticks=[0,1,2],ylabel='Absolute count shift',xlabel='Retrieval heads')
ax.legend([Line2D([],[],marker='o',color=INK,ls='',ms=4),Line2D([],[],marker='D',color=INK,ls='',mfc='white',ms=4)],['Top-$K$','Random'],loc='lower center',bbox_to_anchor=(.5,1.025),ncol=2,fontsize=11.5,handletextpad=.35,columnspacing=.8,borderpad=0,borderaxespad=0)
ax=axes[2]
for m,c in zip(MODELS,COLORS):
...
for ax in axes:
    ...
    ax.yaxis.set_label_coords(-.19 if ax is axes[2] else -.15,.5)
fig.canvas.draw()
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v4.{ext}',dpi=220)
...
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v4_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v4.drawio',encoding='utf-8',xml_declaration=True)
...
rows_y={'high':64,'mid':120,'low':176,'tokens':232,'positions':272}
def place(id,cx,cy,w,h,size,bold=False):
    ...
    cell.set('style',style)
for row in rows_y:
    place('row-'+row,134,rows_y[row],200,30,24,True)
...
for index,n in enumerate([1,2,3,10]):
    cx=columns[index]
    place('token-'+str(n),cx,rows_y['tokens'],132,36,28)
    place('low-'+str(n),cx,rows_y['low'],132,42,28)
    place(position_ids[index],cx,rows_y['positions'],132,32,26)
    p=cells['retrieve-branch-'+str(n)].find('.//mxPoint')
    p.set('x',str(cx));p.set('y',str(rows_y['mid']))
for i in range(4):
    place('ellipsis-'+str(i),(columns[i]+columns[i+1])/2,rows_y['tokens'],44,30,26)
    cells['ellipsis-'+str(i)].set('value',r'\(\cdots\)')
place('answer-token',columns[-1],rows_y['tokens'],132,36,28)
place('position-T',columns[-1],rows_y['positions'],132,32,26)
for row in ['low','mid','high']:place('answer-'+row,columns[-1],rows_y[row],132,42,28)
place('total',1510,rows_y['high'],186,36,28)
bank_center=(columns[0]+columns[3])/2
place('form-label',bank_center,(rows_y['tokens']+rows_y['low'])/2,180,30,24)
place('retrieve-label',bank_center,rows_y['mid']-23,200,30,24)
place('consolidate-label',1138,(rows_y['high']+rows_y['mid'])/2,200,30,24)
p=cells['retrieval-path'].find('.//mxPoint');p.set('x',str(columns[0]));p.set('y',str(rows_y['mid']))
p=cells['readout'].find('.//mxPoint');p.set('x',str(columns[-1]+66));p.set('y',str(rows_y['high']))
for c in cells.values():
    if c.get('edge')=='1':c.set('style',c.get('style').replace('strokeWidth=3.75','strokeWidth=2.5').replace('endSize=12','endSize=6.5'))
tree.write(OUT/'mechanism_matched_v4.drawio',encoding='utf-8',xml_declaration=True)
model.set('pageWidth','1700');model.set('pageHeight','827')
...
svg=(OUT/'result_panels_v4_outlined.svg').read_bytes()
uri='data:image/svg+xml,'+base64.b64encode(svg).decode()
vertex('section-results','',f'shape=image;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;image={uri};',0,300,1700,527)
tree.write(OUT/'nonthinking_section_v4.drawio',encoding='utf-8',xml_declaration=True)
...
manifest['layout']={'version':4,'result_axes':axis_rects,'axis_font_pt':12,'tick_font_pt':11.5,'legend_font_pt':11.5,'title_font_pt':13,'font_reference_width_inches':10,'at_7_inch_width':{'axis_pt':8.4,'ticks_and_legend_pt':8.05,'title_pt':9.1},'mechanism_column_centers':columns,'mechanism_row_centers':rows_y,'mechanism_text_size':24,'mechanism_math_size':28,'composition':{'width':1700,'mechanism_height':300,'results_height':527},'change_from_v3':{'mechanism_height_reduction':1-300/465,'result_axes_height_increase':(3.1*.61)/(2.6*.57)-1,'tick_and_legend_font_increase':.15}}
...
manifest['layout']['render_check']={'text_outside_canvas':outside,'text_overlaps':overlaps}
