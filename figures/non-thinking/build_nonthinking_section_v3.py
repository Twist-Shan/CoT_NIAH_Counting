...
Run with python. Then export nonthinking_section_v3.drawio.
...
plt.rcParams.update({'font.family':'Times New Roman','font.size':10,'mathtext.fontset':'stix','axes.labelsize':11,'axes.titlesize':12,'xtick.labelsize':10,'ytick.labelsize':10,'axes.linewidth':.7,'lines.linewidth':1.7,'pdf.fonttype':42,'svg.fonttype':'none','text.color':INK,'axes.labelcolor':INK,'legend.frameon':False})
fig=plt.figure(figsize=(10,2.60),facecolor='white')
panel_starts=[.012,.341,.670]
axis_rects=[[x+.070,.200,.240,.57] for x in panel_starts]
...
for x,letter,title in zip(panel_starts,'BCD',['Source-state corruption','Head ablation','Answer-state patching']):
...
ax.set(xlim=(-.5,41.5),ylim=(-.45,3.35),xticks=[0,10,20,30,40],yticks=[0,1,2,3],xlabel='Intervention layer',ylabel='Expected-count\nerror increase')
model_handles=[Line2D([],[],color=c) for c in COLORS]
ax.legend(model_handles,['Qwen','Gemma'],loc='lower center',bbox_to_anchor=(.5,1.02),ncol=2,fontsize=10,handlelength=1.6,borderpad=0)
ax=axes[1]
...
ax.set(xlim=(-.5,1.5),ylim=(-.08,2.5),xticks=[0,1],xticklabels=['Qwen ($K=32$)','Gemma ($K=6$)'],yticks=[0,1,2],ylabel='Absolute count shift',xlabel='Retrieval heads')
ax.legend([Line2D([],[],marker='o',color=INK,ls='',ms=4),Line2D([],[],marker='D',color=INK,ls='',mfc='white',ms=4)],['Top-$K$','Random'],loc='lower center',bbox_to_anchor=(.5,1.02),ncol=2,fontsize=10,handletextpad=.35,columnspacing=.8,borderpad=0)
ax=axes[2]
for m,c in zip(MODELS,COLORS):
...
ax.legend(model_handles,['Qwen','Gemma'],loc='lower center',bbox_to_anchor=(.5,1.02),ncol=2,fontsize=10,handlelength=1.6,borderpad=0)
for ax in axes:
    ax.xaxis.set_label_coords(.5,-.18)
    ax.yaxis.set_label_coords(-.15,.5)
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v3.{ext}',dpi=220)
...
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v3_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v3.drawio',encoding='utf-8',xml_declaration=True)
# Explicit grid: equal source/token column pitch and equal computational row pitch.
cells={c.get('id'):c for c in root.findall('mxCell')}
columns=[330,566,802,1038,1274]
rows_y={'high':90,'mid':185,'low':280,'tokens':375,'positions':430}
def place(id,cx,cy,w,h,size,bold=False):
    ...
    cell.set('style',style)
for row in rows_y:
    place('row-'+row,134,rows_y[row],200,40,28,True)
...
for index,n in enumerate([1,2,3,10]):
    cx=columns[index]
    place('token-'+str(n),cx,rows_y['tokens'],144,48,32)
    place('low-'+str(n),cx,rows_y['low'],144,56,32)
    place(position_ids[index],cx,rows_y['positions'],144,42,30)
    p=cells['retrieve-branch-'+str(n)].find('.//mxPoint')
    p.set('x',str(cx));p.set('y',str(rows_y['mid']))
for i in range(4):
    place('ellipsis-'+str(i),(columns[i]+columns[i+1])/2,rows_y['tokens'],44,40,30)
    cells['ellipsis-'+str(i)].set('value',r'\(\cdots\)')
place('answer-token',columns[-1],rows_y['tokens'],144,48,32)
place('position-T',columns[-1],rows_y['positions'],144,42,30)
for row in ['low','mid','high']:place('answer-'+row,columns[-1],rows_y[row],144,56,32)
place('total',1510,rows_y['high'],200,48,32)
bank_center=(columns[0]+columns[3])/2
place('form-label',bank_center,(rows_y['tokens']+rows_y['low'])/2,180,36,28)
place('retrieve-label',bank_center,rows_y['mid']-29,200,36,28)
place('consolidate-label',1138,(rows_y['high']+rows_y['mid'])/2,200,36,28)
...
for c in cells.values():
    if c.get('edge')=='1':c.set('style',c.get('style').replace('strokeWidth=3.75','strokeWidth=3.2').replace('endSize=12','endSize=10'))
tree.write(OUT/'mechanism_matched_v3.drawio',encoding='utf-8',xml_declaration=True)
model.set('pageWidth','1700');model.set('pageHeight','940')
...
vertex('section-title-a','Non-thinking mechanism','text;html=1;strokeColor=none;fillColor=none;fontFamily=Times New Roman;fontSize=28.33;fontStyle=1;fontColor=#30312E;align=left;',64,0,820,44)
svg=(OUT/'result_panels_v3_outlined.svg').read_bytes()
uri='data:image/svg+xml,'+base64.b64encode(svg).decode()
vertex('section-results','',f'shape=image;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;image={uri};',0,465,1700,442)
tree.write(OUT/'nonthinking_section_v3.drawio',encoding='utf-8',xml_declaration=True)
...
manifest['layout']={'version':3,'result_axes':axis_rects,'axis_font_pt':11,'tick_font_pt':10,'legend_font_pt':10,'title_font_pt':12,'font_reference_width_inches':10,'at_7_inch_width':{'axis_pt':7.7,'ticks_and_legend_pt':7,'title_pt':8.4},'mechanism_column_centers':columns,'mechanism_row_centers':rows_y,'mechanism_text_size':28,'mechanism_math_size':32}
