...
Run with python, then export_nonthinking_section.ps1.
The export script composes the final PDF with selectable result-panel text.
...
# Single-layer source-count matching on pairs with correct baseline answers.
import importlib.util
analysis_path = Path(__file__).resolve().parents[1] / 'answer_state_source_count_20260921/analyze.py'
spec = importlib.util.spec_from_file_location('source_count_analysis', analysis_path)
source_count = importlib.util.module_from_spec(spec)
spec.loader.exec_module(source_count)
curves, _, answer_audit = source_count.compute(REPO/'exports')
answer = [dict(r, layer=r['layer']-1, seed_count=r['seeds']) for r in curves if r['condition']=='donor_transport']
for relative in answer_audit['input_sha256']:
    record(REPO/'exports'/relative)
writecsv(DATA/'answer_patching_plot.csv',answer)
(DATA/'answer_patching_audit.json').write_text(json.dumps(answer_audit,indent=2),encoding='utf-8')

plt.rcParams.update({'font.family':'Times New Roman','font.size':12,'mathtext.fontset':'stix','axes.labelsize':13,'axes.titlesize':16.3,'xtick.labelsize':12,'ytick.labelsize':12,'axes.linewidth':.8,'lines.linewidth':1.9,'pdf.fonttype':42,'svg.fonttype':'none','text.color':INK,'axes.labelcolor':INK,'legend.frameon':False})
...
panel_headers=[]
for x,letter,title in zip(panel_starts,'BCD',['Record-span patching','Head ablation','Answer state patching']):
    panel_headers.append([
        fig.text(x,.967,letter,fontweight='bold',fontsize=15.75,va='top'),
        fig.text(x+.026,.967,title,fontweight='bold',fontsize=16.3,va='top')])
ax=axes[0]
for m,c in zip(MODELS,COLORS):
    for direction,linestyle in [('restore','-'),('corrupt',(0,(3.5,2.2)))]:
        ...
        x=[int(r['layer'])+1 for r in rr];y=[float(r['mean']) for r in rr]
...
ax.set(xlim=(.5,42.5),ylim=(-.09,.69),xticks=[1,11,21,31,41],yticks=[0,.2,.4,.6],xlabel='Intervention layer',ylabel='Effect')
ax.yaxis.set_major_formatter(PercentFormatter(1,decimals=0))
model_handles=[Line2D([],[],color=c) for c in COLORS]
model_legend=ax.legend(model_handles,['Qwen','Gemma'],loc='lower center',bbox_to_anchor=(.5,1.025),ncol=2,fontsize=13,handlelength=1.6,borderpad=0,borderaxespad=0)
ax.add_artist(model_legend)
ax.legend([Line2D([],[],color=INK,ls='-'),Line2D([],[],color=INK,ls=(0,(3.5,2.2)))],
          ['Restore','Corrupt'],loc='upper right',bbox_to_anchor=(.99,.99),fontsize=12,
...
for i,(m,c) in enumerate(zip(MODELS,COLORS)):
    ...
    for cond,marker in [('original','o'),('layer_matched_random','D'),('ranked','o')]:
        ...
        if cond!='original':
            ...
            ax.annotate(f'{100*y:.0f}%',(i,y),xytext=(dx,5 if cond=='ranked' else 2),textcoords='offset points',color=c,ha=align,va='bottom',fontsize=11.5)
...
for i,k in enumerate([32,6]):
    ax.text(i,-.22,f'$K={k}$',transform=ax.get_xaxis_transform(),ha='center',va='top',fontsize=13)
ax.legend([Line2D([],[],marker='o',color=INK,ls='',mfc='white',ms=4),Line2D([],[],marker='D',color=INK,ls='',ms=4),Line2D([],[],marker='o',color=INK,ls='',ms=4)],['Original','Random','Top-$K$'],loc='lower center',bbox_to_anchor=(panel_starts[1]+panel_widths[1]/2,.210+.57*1.025),bbox_transform=fig.transFigure,ncol=3,fontsize=13,handlelength=.4,handletextpad=.34,columnspacing=.42,borderpad=0,borderaxespad=0)
ax=axes[2]
for m,c in zip(MODELS,COLORS):
    rr=[r for r in answer if r['model']==m]; x=[r['layer']+1 for r in rr]
...
ax.set(xlim=(.5,42.5),ylim=(-.04,1.08),xticks=[1,11,21,31,41],yticks=[0,.2,.4,.6,.8,1],xlabel='Intervention layer',ylabel='Source-count match rate')
ax.yaxis.set_major_formatter(PercentFormatter(1,decimals=0))
ax.legend(model_handles,['Qwen','Gemma'],loc='lower center',bbox_to_anchor=(.5,1.025),ncol=2,fontsize=13,handlelength=1.6,borderpad=0,borderaxespad=0)
for ax in axes:
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v11.{ext}',dpi=220)
# draw.io's SVG image renderer cannot resolve Matplotlib's local STIX font.
# Use outlined glyphs in the editable draw.io preview to avoid font fallback.
# export_nonthinking_section.ps1 replaces these with the native PDF for delivery.
with plt.rc_context({'svg.fonttype':'path'}):
    fig.savefig(OUT/'result_panels_v11_outlined.svg')
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v11.drawio',encoding='utf-8',xml_declaration=True)
...
rows_y={'high':72,'mid':148,'low':224,'tokens':306,'positions':352}
def place(id,cx,cy,w,h,size,bold=False):
    ...
    cell.set('style',style)
for row in rows_y:
    place('row-'+row,134,rows_y[row],200,36,30.6,True)
...
for index,n in enumerate([1,2,3,10]):
    cx=columns[index]
    place('token-'+str(n),cx,rows_y['tokens'],144,44,33.2)
    place('low-'+str(n),cx,rows_y['low'],80,56,33.2)
    place(position_ids[index],cx,rows_y['positions'],144,38,30.6)
...
place('answer-token',columns[-1],rows_y['tokens'],144,44,33.2)
place('position-T',columns[-1],rows_y['positions'],144,38,30.6)
for row in ['low','mid','high']:place('answer-'+row,columns[-1],rows_y[row],80,56,33.2)
# MathJax superscripts/subscripts exceed the old 44-pixel routing box.
# A taller box and optical vertical adjustment keep arrowheads clear of the ink.
for math_id in ['low-1','low-2','low-3','low-10','answer-low','answer-mid','answer-high']:
    cell=cells[math_id]
    cell.set('style',cell.get('style')+'spacingBottom=10;')
place('total',1510,rows_y['high'],200,44,33.2)
bank_center=(columns[0]+columns[3])/2
place('form-label',bank_center,(rows_y['tokens']+rows_y['low'])/2,180,34,30.6)
stage_label_y=(rows_y['high']+rows_y['mid'])/2
place('retrieve-label',bank_center,stage_label_y,200,34,30.6)
place('consolidate-label',1110,stage_label_y,200,34,30.6)
...
cells['readout'].set('source','answer-high')
readout_geometry=cells['readout'].find('mxGeometry')
for point in list(readout_geometry):readout_geometry.remove(point)
for c in cells.values():
    if c.get('edge')=='1':c.set('style',c.get('style').replace('strokeWidth=3.75','strokeWidth=2.5').replace('endSize=12','endSize=6.5'))
for edge_id in ['retrieve-branch-1','retrieve-branch-2','retrieve-branch-3','retrieve-branch-10','query-upward','consolidation']:
    cell=cells[edge_id]
    cell.set('style',cell.get('style').replace('exitDy=0;','exitDy=-6;'))
tree.write(OUT/'mechanism_matched_v11.drawio',encoding='utf-8',xml_declaration=True)
...
vertex('section-panel-a','A','text;html=1;strokeColor=none;fillColor=none;fontFamily=Times New Roman;fontSize=37.19;fontStyle=1;fontColor=#30312E;align=left;',20,0,35,44)
vertex('section-title-a','Non-thinking mechanism','text;html=1;strokeColor=none;fillColor=none;fontFamily=Times New Roman;fontSize=38.52;fontStyle=1;fontColor=#30312E;align=left;',64,0,820,44)
svg=(OUT/'result_panels_v11_outlined.svg').read_bytes()
...
tree.write(OUT/'nonthinking_section_v11.drawio',encoding='utf-8',xml_declaration=True)
manifest={'status':'LAYOUT_DRAFT_PENDING_EXACT_CORRUPTION_CSV' if is_draft else 'COMPLETE','sources':sources,'palette':dict(zip(MODELS,COLORS)),'mechanism_palette':mapping,'corruption':{'population':'confirmation 10 seeds x counts 1-10','layers':[36,42],'precision':'3 decimal places; no reconstructed CI' if is_draft else 'exact source CSV'},'ablation':{'seed_clusters':20,'metric':'absolute generated-count shift','gemma_top6':'post-hoc dose extension'},'answer':{'seed_counts':sorted({r['seed_count'] for r in answer}),'protocol':'single_layer; both baseline answers correct','metric':'source_count_match_rate','bootstrap':10000},'editable':'mechanism native draw.io cells; quantitative panel SVG, regenerated by this script'}
manifest['layout']={'version':6,'result_axes':axis_rects,'axis_font_pt':13,'tick_font_pt':12,'legend_font_pt':13,'title_font_pt':16.3,'font_reference_width_inches':10,'at_6_5_inch_width':{'axis_pt':8.45,'tick_pt':7.8,'model_legend_pt':8.45,'direction_legend_pt':7.8,'annotation_pt':7.475,'title_pt':10.595,'panel_letter_pt':10.2375},'mechanism_column_centers':columns,'mechanism_row_centers':rows_y,'mechanism_text_size':30.6,'mechanism_math_size':33.2,'composition':{'width':1700,'mechanism_height':380,'results_height':450.5},'change_from_v5':{'upper_row_pitch':[50,70],'answer_arrow_clear_length':[14,26],'low_to_token_pitch':[82,80],'token_to_position_pitch':[50,54],'mechanism_text_size':[24,28],'mechanism_math_size':[26,32],'results_layout':'same plot widths/heights; full panel bounds aligned and equally spaced'}}
manifest['layout']['font_export']='Final PDF combines the original mechanism with the native result PDF, retaining embedded Times New Roman/STIX fonts and selectable result text; draw.io previews retain outlined result glyphs to prevent font fallback'
...
manifest['layout']['version']=11
manifest['layout']['arrow_alignment']={'math_routing_box':[80,56],'math_spacing_bottom':10,'upward_source_clearance':6,'vertical_centerlines':columns,'retrieval_bus_y':rows_y['mid'],'readout_y':rows_y['high'],'answer_vertical_arrow_gap':14,'readout_source':'answer-high east-center port'}
...
# \gpt: v11 aligns printed font sizes with the overview figure; data and aligned arrow endpoints retained; row spacing adjusted for larger fonts.
manifest['terminology']={'patch_roles':'source provides states; target receives states', 'panel_B_title':'Record-span patching','previous_version':9}
manifest['layout']['font_alignment']={'reference':'runs/paper_figures/figures/main_figure_v4.pdf','reference_design_width':1280,'paper_width_inches':6.5,'main_title_pt':10.603125,'main_body_pt':8.409375,'main_tick_pt':7.4953125,'previous_version':10,'mechanism_title_size':38.52,'mechanism_letter_size':37.19,'mechanism_row_adjustments':{'high':16,'mid':12,'low':8,'tokens':4,'positions':0},'annotation_font_pt':11.5,'direction_legend_font_pt':12}
