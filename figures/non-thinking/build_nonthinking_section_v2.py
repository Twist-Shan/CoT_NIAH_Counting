...
Run with python. Then export nonthinking_section_v2.drawio.
...
fig=plt.figure(figsize=(10,2.70),facecolor='white')
...
for ext in ['pdf','svg','png']:fig.savefig(OUT/f'result_panels_v2.{ext}',dpi=220)
plt.close(fig)
...
tree.write(OUT/'mechanism_matched_v2.drawio',encoding='utf-8',xml_declaration=True)
...
for cell in root.findall('mxCell'):
    ...
    if geo is not None:
        if cell.get('vertex')=='1':
            ...
            original_y=float(geo.get('y','0'))
            center=.62*(original_y+h/2)+35
            if original_y>=574:center+=14
            # Keep glyph sizes; shrink the padding boxes so arrows retain shafts.
            new_h=54 if h==80 else (48 if h==64 else h)
            geo.set('height',str(new_h))
            geo.set('y',str(center-new_h/2))
        if cell.get('edge')=='1':
            for point in geo.iter('mxPoint'):
                if 'y' in point.attrib:point.set('y',str(.62*float(point.get('y'))+35))
...
svg=(OUT/'result_panels_v2.svg').read_bytes()
uri='data:image/svg+xml,'+base64.b64encode(svg).decode()
vertex('section-results','',f'shape=image;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;image={uri};',0,465,1700,459)
tree.write(OUT/'nonthinking_section_v2.drawio',encoding='utf-8',xml_declaration=True)
