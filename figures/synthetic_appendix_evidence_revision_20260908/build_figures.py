...
PAPER=WORK/'runs/paper_figures/figures/synthetic_appendix/evidence_revision_20260908'
...
def save(fig,stem,title):
    ...
    CHECKS.append(inspect_figure(fig,stem))
...
def box(ax,xy,w,h,text,color=INK,fill='#F5F6F9',size=10):
    ...
    ax.text(xy[0]+w/2,xy[1]+h/2,text,ha='center',va='center',fontsize=size,
            color=INK,transform=ax.transAxes,linespacing=1.3)
...
def training_behavior():
    loss=read(OLD/'plot_data/01_evaluation_loss.csv')
    behavior=read(OLD/'plot_data/01_behavior_by_count.csv')
    export(loss,'01_evaluation_loss.csv'); export(behavior,'01_behavior_by_count.csv')
    assert behavior.inputs.eq(10).all()
    fig=plt.figure(figsize=(6.5,2.55))
    for i,mode in enumerate(['nonthinking','thinking','both']):
        ax=fig.add_axes([(.49+i*2.15)/6.5,.30,1.65/6.5,.52])
        if i<2:
            for curve,ls in [('train','-'),('heldout','--')]:
                f=loss[loss['mode'].eq(mode)&loss.curve_source.eq(curve)].sort_values('step')
                ax.plot(f.step,f.token_weighted_cross_entropy,color=NT if i==0 else T,ls=ls)
            ax.axvline(1500,color=GRAY,ls=':',lw=1)
            ax.set(xlabel='Training step',xticks=[0,5000,10000],xticklabels=['0','5k','10k'],xlim=(0,10000))
            panel(ax,['A. Non-thinking loss','B. Thinking loss'][i],'Cross-entropy')
        else:
            for m,color in [('nonthinking',NT),('thinking',T)]:
                f=behavior[behavior['mode'].eq(m)].sort_values('count')
                ax.plot(f['count'],100*f.accuracy,'o-',color=color,ms=3.5)
            panel(ax,'C. Answer accuracy','Accuracy (%)')
            ax.set(xlabel='True count',xticks=range(1,11),xlim=(.6,10.4),ylim=(-3,103),yticks=[0,25,50,75,100])
    fig.legend([Line2D([],[],color=NT,marker='o'),Line2D([],[],color=T,marker='o'),
                Line2D([],[],color=INK,ls='-'),Line2D([],[],color=INK,ls='--')],
               ['Non-thinking','Thinking','Training region','Validation region'],
               loc='lower center',bbox_to_anchor=(.52,.015),ncol=4,fontsize=9,
               handlelength=1.4,columnspacing=1.,handletextpad=.35)
...
def transport():
    ...
    a.set_title('A. Value activation patching',loc='left',fontsize=11,pad=8)
    box(a,(.05,.76),.38,.18,'Clean source: a',INK,'#F5F6F9')
    box(a,(.56,.76),.38,.18,'Corrupted: b',ORANGE,'#FFF5ED')
    arrow(a,(.44,.85),(.55,.85))
    box(a,(.16,.33),.68,.24,'Keep b in the input\nRestore only clean V(a)',T,'#EDF7FA')
    arrow(a,(.75,.74),(.62,.58),ORANGE)
    arrow(a,(.24,.74),(.37,.58),INK)
    a.text(.5,.14,'Next-token logits: a versus b',ha='center',va='center',fontsize=10)
    a.text(.5,-.02,'Example: target count unchanged',ha='center',fontsize=9,color=INK)
    b=fig.add_axes([.62,.37,.36,.49])
    for cond,color,style,label in [('value_selected',T,'-','Selected heads'),('value_control',GRAY,'--','Matched controls')]:
        g=d[d.condition.eq(cond)].groupby('top_k').margin.mean()
        b.plot(g.index,g.values,'o'+style,color=color,label=label,ms=4)
    for cond,color,label in [('clean',INK,'Clean'),('damaged',ORANGE,'Corrupted')]:
        b.axhline(d.loc[d.condition.eq(cond),'margin'].iloc[0],color=color,ls=':',label=label,lw=1.4)
    b.axhline(0,color=INK,lw=.65)
    panel(b,'B. Logit difference',r'Logit(a) $-$ logit(b)')
    b.set(xlabel='Patched heads',xticks=[1,2,4],xlim=(.85,4.15),ylim=(-5,11),yticks=[-4,0,4,8])
...
def readability():
    ...
    for j,ax in enumerate(axes):
        ...
        panel(ax,['A. Occurrence index','B. Final count'][j],'Balanced accuracy (%)')
    ...
    save(fig,'04_count_readability','Count decoding across layers')
...
def geometry():
    ...
    for i,(mode,answer,title) in enumerate(specs):
        ...
        fig.text(x+.01,y+.395,title,fontsize=11,color=NT if mode=='nonthinking' else T)
...
def sources():
    ...
    for ctrl,off,color,label in [(False,-w/2,T,'Source ablation'),(True,w/2,GRAY,'Control ablation')]:
...
def continuation():
    ...
    ax=fig.add_axes([.075,.29,.37,.54]); x=np.arange(4); w=.34
    ...
    panel(ax,'A. Continuation exact match','Exact match (%)')
    ax.set(xticks=x,xticklabels=['1','2','3','4'],xlabel='Number of subsequent markers',ylim=(0,60),yticks=[0,20,40,60])
    fig.legend(*ax.get_legend_handles_labels(),loc='lower left',bbox_to_anchor=(.065,.005),ncol=2,
               fontsize=9,handlelength=1.2,columnspacing=.9)
    ax=fig.add_axes([.60,.29,.37,.54])
    effect=100*d.effect.to_numpy()
    err=np.vstack([effect-100*d.ci_low.to_numpy(),100*d.ci_high.to_numpy()-effect])
    ax.errorbar(x,effect,yerr=err,fmt='o',color=T,ecolor=T,capsize=4,elinewidth=1.4,ms=5)
    ax.axhline(0,color=INK,lw=.9)
    panel(ax,'B. Paired difference (95% CI)','Difference (percentage points)')
    ax.set(xticks=x,xticklabels=['1','2','3','4'],xlabel='Number of subsequent markers',
           xlim=(-.5,3.5),ylim=(-15,60),yticks=[-10,0,20,40,60])
    fig.text(.785,.047,'Donor state minus orthogonal control',ha='center',fontsize=9)
    CHECKS.append(dict(figure='07_progress_continuation',interval_estimand='prompt-averaged paired difference',
                       archived_intervals=True,bootstrap_resamples=10000,bootstrap_unit='input',eligible_inputs=8))
    save(fig,'07_progress_continuation','Does a transplanted item change the continuation?')

def answer():
    a=read(OLD/'plot_data/07_answer_sources.csv'); b=read(OUT/'plot_data/08_donor_metrics.csv')
    export(a,'08_answer_sources_all_modes.csv'); export(b,'08_answer_state_donor.csv')
    fig=plt.figure(figsize=(6.5,3.05))
    axs=[fig.add_axes([left,.34,.242,.49]) for left in [.082,.418,.746]]
    ax=axs[0]; x=np.arange(2); w=.31
    f=a[a['mode'].eq('thinking')]
    for arms,off,color,label in [(['records','trace'],-w/2,T,'Source ablation'),
                                 (['ordinary_records_budget','ordinary_trace_budget'],w/2,GRAY,'Control ablation')]:
        vals=[100*f.loc[f.arm.eq(arm),'accuracy'].iloc[0] for arm in arms]
        bars=ax.bar(x+off,vals,w,color=color,label=label)
        ax.bar_label(bars,fmt='%.0f',padding=2,fontsize=9)
    ax.axhline(95,color=INK,ls=':',lw=1,label='Clean (95%)')
    panel(ax,'A. Thinking: sources','Answer accuracy (%)')
    ax.set(xticks=x,xticklabels=['Prompt\ntargets','Generated\ntrace'],ylim=(-4,115),yticks=[0,25,50,75,100])
    handles,labels=ax.get_legend_handles_labels()
    fig.legend(handles,labels,loc='lower left',bbox_to_anchor=(.06,.005),ncol=1,
               fontsize=9,handlelength=1.2,handletextpad=.35,labelspacing=.25)
    for ax,metric,title in zip(axs[1:],['truth','prediction'],['B. Donor ground truth','C. Donor prediction']):
        for mode,color,marker,label in [('nonthinking',NT,'s','Non-thinking'),('thinking',T,'o','Thinking')]:
            f=b[b['mode'].eq(mode)].sort_values('layer')
            assert f.pairs.eq(180).all()
            ax.plot(f.layer,100*f['patched_match_'+metric],marker+'-',color=color,label=label,
                    mfc='white' if mode=='thinking' else color,ms=4.5)
            ax.plot(f.layer,100*f['baseline_match_'+metric],':',color=color,lw=1.4)
        panel(ax,title,'Match (%)')
        ax.set(xticks=[1,2,3,4],xlabel='Patching layer',xlim=(.85,4.15),ylim=(-4,115),yticks=[0,25,50,75,100])
    fig.legend([Line2D([],[],color=NT,marker='s'),Line2D([],[],color=T,marker='o',mfc='white'),
                Line2D([],[],color=INK,ls='-'),Line2D([],[],color=INK,ls=':')],
               ['Non-thinking','Thinking','After patching','Before patching'],
               loc='lower left',bbox_to_anchor=(.405,.015),ncol=2,fontsize=9,
               handlelength=1.6,columnspacing=1.,handletextpad=.45)
...
def attention_dynamics():
    ...
    for i,(mode,role,title) in enumerate([
        ...
        images.append(im)
        ax.set_title(title,loc='left',fontsize=11,pad=7,linespacing=1.2)
        ax.set(ylim=(31.5,-.5),xlim=(0,10000),xticks=[0,5000,10000],xticklabels=['0','5k','10k'])
        ax.set_yticks(range(0,32,4),[f'L{l}H{h+1}' for l,h in heads[::4]] if i==0 else [])
        ax.tick_params(axis='y',length=0,pad=3,labelsize=9)
...
def metric_dynamics():
    ...
    for i,title in enumerate(['A. Occurrence index','B. Final count','C. Value patching']):
        ax=fig.add_axes([(.53+i*2.11)/6.5,.29,1.60/6.5,.52])
        if i<2:
            ...
            ylabel='Logit-difference change'
        ...
        ax.legend(loc=['upper right','center right','upper left'][i],fontsize=9,handlelength=1.,
...
def main():
    ...
    for p in [WORK/'runs/paper_figures/main.tex',WORK/'runs/paper_figures/appendix/synthetic.tex',
        ...
        source(p)
    source(WORK/'figures/synthetic_appendix_paper_20260908/appendix_layout_preview.pdf')
    training_behavior()
    retrieval(); transport()
    readability()
    ...
    manifest=dict(figures=FIGURES,source_sha256=SOURCES,checks=CHECKS,model_runs=False,
                  manuscript_modified_by_builder=False,old_files_preserved=True,camera=CAMERA,
                  palette={'nonthinking':NT,'thinking':T,'control':GRAY,'clean':INK,'corrupted':ORANGE,
                           'donor_annotation':VIOLET,'count_cmap':['#23165C','#6750E8','#00C2FF','#39E58C','#F6E36A'],
                           'attention_cmap':['#161923','#40204F','#963878','#E65D91','#F7A35C','#F9EDAD']},
                  typography_pt={'minimum':9,'tick':9,'legend':9,'axis_label':10,'panel_title':11},
                  output_dpi=450,figure_width_in=6.5,
