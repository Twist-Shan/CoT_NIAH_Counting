...
def main():
    ...
    for ax, (source, target, title, expected_rows, expected_seeds) in zip(axes, PANELS):
        for model, short, color, layers in MODELS:
            path = (PREVIOUS / f'classification_prompt_20_{short}/answer_classifier_metrics.csv'
            ...
            for method, style in METHODS:
                ...
                if method == 'nearest_centroid':
                    choice = SELECTION[model]['running_index' if source == 'prompt' else 'answer_query_scan']
                    ax.scatter(choice['layer'] + 1, choice['ncc'], s=27, color=color,
                               edgecolor='white', linewidth=.6, zorder=4)
                    ax.annotate(f"L{choice['layer'] + 1}", (choice['layer'] + 1, choice['ncc']),
    ...
    manifest = {
        ...
        'refit_audit': '../nonthinking_count_readouts_20260912/running_index_20_audit.json',
    ...
    (OUT / 'readouts_manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
