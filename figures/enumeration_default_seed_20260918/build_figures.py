"""Render audited default-seed Enumeration results with the existing paper style."""
...
PREVIOUS=OUT
REPORTS=REPO/'outputs/enumeration_default_seed_20260918/reports/audit_v2'
...
def head_scores():
    ...
    for mode,source,name in [('enumeration_bullet',bullet,'enumeration_head_scores')]:
...
def dose():
    ...
    normalized={'enumeration_retrieve':bullet}
    ...
    for name,rows in normalized.items():
        ...
        save(fig,name,rows,dict(model_rows=['Qwen','Gemma'],outcome_columns=['next item','final count'],statistics_recomputed=True))

def update():
    report=js(REPORTS/'update_audit.json')
...
def prepare_data():
    ...
    reports={k:js(REPORTS/f'{k}_audit.json') for k in ['read','retrieve','answer','update']}
    assert js(REPORTS/'audit.json')['status']=='PASS'
    assert all(v['status']=='PASS' for v in reports.values())
    arrays={k:[] for k in ['enumeration_head_scores','enumeration_retrieve','enumeration_readout']}
    for model,_,_,_ in MODELS:
        ranking=js(REPORTS.parent/'retrieve/jobs/localize'/model/'enumeration_bullet/ranking.json')
        arrays['enumeration_head_scores'].extend(dict(v,model=model,mode='enumeration_bullet') for v in ranking)
        for dose in cell(reports['retrieve'],model)['dose_response']:
            for metric,terms in dose['metrics'].items():
                for cond in ['selected_minus_clean','random_minus_clean']:
                    stat=terms[cond]
                    arrays['enumeration_retrieve'].append(dict(model=model,mode='enumeration_bullet',metric=metric,condition=cond,k=dose['dose_k'],estimate=stat['estimate'],ci95=stat['ci95'],random_control=dose['random_control']))
        for layer in cell(reports['answer'],model)['layerwise']:
            for cond in ['self_patch','full_donor_patch']:
                stat=layer['conditions'][cond]['donor_adoption']
                arrays['enumeration_readout'].append(dict(model=model,mode='enumeration_bullet',metric='answer_donor_adoption',condition=cond,layer=layer['layer_one_based'],estimate=stat['estimate'],ci95=stat['ci95']))
        for cond in ['clean','prompt_records_blank','trace_all_blank']:
            stat=cell(reports['read'],model)['conditions'][cond]['accuracy']
            arrays['enumeration_readout'].append(dict(model=model,mode='enumeration_bullet',metric='blanking_accuracy',condition=cond,estimate=stat['value'],ci95=stat['ci95'],numerator=stat['numerator'],denominator=stat['denominator']))
    for name,rows in arrays.items():(OUT/'data'/f'{name}.json').write_text(json.dumps(rows,indent=2)+'\n',encoding='utf-8')
...
if __name__=='__main__':
    ...
    prepare_data();head_scores();dose();update();readout();read(Path(__file__))
    (OUT/'manifest.json').write_text(json.dumps(dict(status='PASS',figures=FIGURES,source_sha256=SOURCES,statistics_recomputed=True,inference=False,font_profile=dict(family='Times New Roman',title=11,axis=10,ticks_legend=9,selected_layer_annotation=8,width_inches=6.5)),indent=2)+'\n')
    print('PASS: four Enumeration figures from audited default-seed statistics.')
